...
class InversorEnBD implements Inversor,Resumible {
    ...
    public InversorEnBD(int id, float capital) {
        this.id = id;
    ...
    public boolean comprar(int id, int cantidade) {
        ...
        try {
            ...
            PreparedStatement statement1 = BolsaEnBD.getCon().prepareStatement("SELECT * FROM usuario_valor INNER JOIN valores on usuario_valor.id_valor=valores.id WHERE id_usuario=? AND id_valor=?");
            ...
            if (resultado1.isBeforeFirst()) {
                while (resultado1.next()) {
                    ...
                    PreparedStatement statement2 = BolsaEnBD.getCon().prepareStatement("UPDATE usuario_valor SET cantidade=? WHERE id_usuario=? AND id_valor=?");
                    statement2.setInt(1, cantidade);
                    statement2.setInt(2, this.getId());
                    statement2.setInt(3, id);
                    statement2.execute();
                    statement2 = BolsaEnBD.getCon().prepareStatement("UPDATE usuarios SET capital=? WHERE id=?");
                ...
                PreparedStatement statement3 = BolsaEnBD.getCon().prepareStatement("SELECT prezo FROM valores WHERE id=?");
                statement3.setInt(1, id);
                ResultSet resultado2 = statement3.executeQuery();
                while (resultado2.next()) {
                    setCapital(getCapital() - cantidade * resultado2.getFloat("prezo"));
                }
                statement3 = BolsaEnBD.getCon().prepareStatement("INSERT INTO usuario_valor VALUES(?,?,?)");
                statement3.setInt(1, this.getId());
                statement3.setInt(2, id);
                statement3.setInt(3, cantidade);
                statement3.execute();
                statement3 = BolsaEnBD.getCon().prepareStatement("UPDATE usuarios SET capital=? WHERE id=?");
        ...
        try {
            ...
            if (resultado1.isBeforeFirst()) {
                while (resultado1.next()) {
                    ...
                    statement2 = BolsaEnBD.getCon().prepareStatement("SELECT cantidade FROM usuario_valor WHERE id_usuario=? AND id_valor=?");
                    statement2.setInt(1, this.getId());
                    statement2.setInt(2, id);
                    ResultSet rs = statement2.executeQuery();
                    while (rs.next()) {
                        if (rs.getInt("cantidade") == 0) {
                            statement2 = BolsaEnBD.getCon().prepareStatement("DELETE FROM usuario_valor WHERE id_usuario=? AND id_valor=?");
                            statement2.setInt(1, this.getId());
                            statement2.setInt(2, id);
                            statement2.execute();
                        }
